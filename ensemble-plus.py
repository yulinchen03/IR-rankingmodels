...
from IRutils.inference_plus import (
    evaluate,
...
def run(model_name, dataset_name, metrics, device, length_setting='full', max_len_doc=512, random_state=42):
    ...
    # Now that embeddings are guaranteed to exist, we can proceed with evaluations,
    ...
    for metric in metrics:
        print(f'  Metric {metric}: {baseline_metrics[metric]:.4f}')
    results['baseline'] = baseline_metrics

    # Save baseline results
    save_path_baseline = os.path.join(results_save_dir, 'baseline.txt')
    ...
    print("Average Ensemble Performance:")
    for metric in metrics:
        print(f'  Metric {metric}: {avg_metrics[metric]:.4f}')
    results['ens-avg'] = avg_metrics

    save_path_avg = os.path.join(results_save_dir, 'ensemble-avg.txt')
    write_results(avg_metrics, save_path_avg, model_name, dataset_name, length_setting)
    print(f"Average ensemble results saved to {save_path_avg}")
    ...
    print("Conditional Ensemble Performance:")
    for metric in metrics:
        print(f'  Metric {metric}: {cond_metrics[metric]:.4f}')
    results['ens-select'] = cond_metrics

    save_path_cond = os.path.join(results_save_dir, 'ensemble-selective.txt')
    write_results(cond_metrics, save_path_cond, model_name, dataset_name, length_setting)
    print(f"Conditional ensemble results saved to {save_path_cond}")
    ...
    print("Fixed Weighted Ensemble Performance:")
    for metric in metrics:
        print(f'  Metric {metric}: {weighted_metrics[metric]:.4f}')
    results['ens-weighted'] = weighted_metrics

    save_path_weighted = os.path.join(results_save_dir, 'ensemble-weighted.txt')
    write_results(weighted_metrics, save_path_weighted, model_name, dataset_name, length_setting)
    print(f"Fixed weighted ensemble results saved to {save_path_weighted}")
    ...
    # If precompute_validation_scores needs its own memory management, refactor it too.
    precomputed_val_scores = precompute_validation_scores(models, val_loader, device)

    weight_opt_trials = 1000
    ...
    learned_weighted_metrics, learned_weighted_per_query, learned_weighted_run = evaluate_weighted_average_ensemble(
        models=models,
        tokenizer=tokenizer,
        queries=eval_queries, # Use TEST queries
        embedding_save_dir=embedding_save_dir, # Pass DIR
        weights_config=learned_weights_config, # Use LEARNED weights
        qrels=eval_qrels, # Use TEST qrels
        t1=t1, t2=t2,
        device=device,
        max_len_doc=max_len_doc
    )

    print("\nLearned Weighted Ensemble Performance (Test Set):")
    for metric in metrics:
         print(f'  Metric {metric}: {learned_weighted_metrics[metric]:.4f}')
    results['ens-learned-weighted'] = learned_weighted_metrics

    save_path_learned = os.path.join(results_save_dir, 'ensemble-learned_weighted.txt')
    write_results(learned_weighted_metrics, save_path_learned, model_name, dataset_name, length_setting)
    print(f"Learned weighted ensemble results saved to {save_path_learned}")
    # Memory cleanup inside evaluate_weighted_average_ensemble
    # ---------------------------------------------------------------
    ...
    if baseline_metrics:
         ...
         print(ttest_df)
         # print(ttest_df[[Model    Metric  Baseline Score  Model Score  Difference  Improvement %  T-Statistic       P-Value  Significant  Improvement]])
    else:
...
if __name__ == "__main__":
    ...
    metrics = [
        nDCG@100,
        R@100,
        RR # Often RR is calculated without specific K, assuming @10 if needed
    ]
